...
def display_instructions():
    instruction_window = pygame.Surface((600, 400))  # Create a new surface for the instructions
    instruction_window.fill((0, 0, 50))  # Dark background for instructions
    
    font = pygame.font.Font("Front/UTM-Birds-Paradise.ttf", 20)  # Load custom font
    ...
    for line in instructions:
        ...
        y_offset += 40  # Space each line 40 pixels apart

    # Main loop to show instructions
    running = True
    while running:
        screen.blit(instruction_window, ((screen_width - 600) // 2, (screen_height - 400) // 2))
        pygame.display.flip()
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if not show_instruction and 0 <= mouse_x <= 200 and screen_height - 80 <= mouse_y <= screen_height - 20:
                show_instruction = True  # Hiển thị bảng thông báo
            elif show_instruction:
                # Kiểm tra nếu click vào nút "Close" trong bảng thông báo
                popup_x = (screen_width - 400) // 2
                popup_y = (screen_height - 300) // 2
                if popup_x + 300 <= mouse_x <= popup_x + 380 and popup_y + 240 <= mouse_y <= popup_y + 280:
                    show_instruction = False 
